...
def filter_df(cur_pitcher, cur_level):
    ...
    filter_criteria = (
        (pads.field("event_code") == 1) |
        (pads.field('event_code') == 6) &
        (pads.field("player_position") == 1)
    )
    ...
    filter_criteria = (
        (pads.field('player_position') == 12) # Positioning for second_baserunner
    )
    ...
    # Assume 2nd Base is at (0, 125)
    query = """
    SELECT
        pitcher AS player_id,
        home_team AS level,
        AVG(SQRT(POWER(field_x, 2) + POWER(field_y - 125, 2))) AS avg_lead,
...
def main():
    conn = sqlite3.connect(':memory:')
    # Parameters
    player_dataframe = pd.read_csv("C:/Users/Hunter Dunn/OneDrive - University of St. Thomas/Documents/GitHub/smt2024umn/Dashboard/pitcher_catcher_db.csv")

    ppo2 = pd.read_csv("C:/Users/Hunter Dunn/OneDrive - University of St. Thomas/Documents/GitHub/smt2024umn/Dashboard/ppo2_db.csv")
    ppo2['level'] = ppo2['game_str'].str.split('_').str[3]

    ppo2.to_sql('ppo2', conn, index=False, if_exists='replace')
    ...
    file_path = "C://Users//Hunter Dunn//OneDrive - University of St. Thomas//Documents//GitHub//smt2024umn//Pull Data//pitcher_pickoff_2nd_lead_tendency.csv"
